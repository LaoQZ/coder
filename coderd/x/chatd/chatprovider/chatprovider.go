...
// anthropicInlineImageCapBytes is Anthropic's documented 5 MiB
// per-inline-image wire limit. The client-side budget in
// site/src/pages/AgentsPage/utils/imageBudget.ts sits slightly
// below this; revisit both if Anthropic revises the limit.
const anthropicInlineImageCapBytes = 5 * 1024 * 1024

// InlineImageCapBytes returns the per-image byte cap an inline image
// part may have for the given provider, or (0, false) when no
// documented cap applies.
//
// Bedrock shares Anthropic's cap because fantasy's bedrock provider
// wraps the anthropic client and uses the same Messages format.
func InlineImageCapBytes(provider string) (int, bool) {
	switch NormalizeProvider(provider) {
	case fantasyanthropic.Name, fantasybedrock.Name:
		return anthropicInlineImageCapBytes, true
	default:
		return 0, false
	}
}
