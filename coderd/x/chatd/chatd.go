...
// chatFileResolver fetches chat files by ID and rejects images that
// exceed the provider's inline image cap (Anthropic / Bedrock-hosted
// Claude) before any upstream request is issued.
//
...
// user-facing warning instead of failing the whole prompt build, but
// that requires altering the FileResolver contract.
