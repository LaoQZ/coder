...
func (s *MethodTestSuite) TestChats() {
	...
	s.Run("GetChatContextClearMessagesByChatID", s.Mocked(func(dbm *dbmock.MockStore, faker *gofakeit.Faker, check *expects) {
		chat := testutil.Fake(s.T(), faker, database.Chat{})
		msgs := []database.ChatMessage{testutil.Fake(s.T(), faker, database.ChatMessage{ChatID: chat.ID})}
		arg := database.GetChatContextClearMessagesByChatIDParams{ChatID: chat.ID, MessageText: "clear"}
		dbm.EXPECT().GetChatByID(gomock.Any(), chat.ID).Return(chat, nil).AnyTimes()
		dbm.EXPECT().GetChatContextClearMessagesByChatID(gomock.Any(), arg).Return(msgs, nil).AnyTimes()
		check.Args(arg).Asserts(chat, policy.ActionRead).Returns(msgs)
	}))
