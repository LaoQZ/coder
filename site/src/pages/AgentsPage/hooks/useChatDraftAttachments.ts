...
export function useChatDraftAttachments(
	...
	// processResize is the async tail of handleAttach: resize the
	// image, then either swap in the smaller file (creating the
	// registry entry and starting the upload) or surface a too-large
	// error. The view starts in "processing" status in handleAttach.
	const processResize = async (
		...
		try {
			...
			// Resize errors fall back to the original below.
			resized = null;
		}

		// Bail if the attachment was removed or the chat scope
		// changed while resizing.
		...
		// Happy path: swap in the replacement (if any), revoke the
		// old blob preview, then start the upload. beginUpload
		// drives the view from pending to uploading via subscribers.
		setViews((prev) =>
			prev.map((view) => {
				if (view.clientId !== clientId) {
					return view;
	...
	const handleAttach = (files: File[]) => {
		...
		for (const file of files) {
			...
			if (file.size > maxAgentAttachmentSize && !needsResize) {
				nextViews.push({
					...baseView,
					status: "error",
