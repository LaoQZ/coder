...
export function useFileAttachments(
	...
	const handleAttach = (files: File[]) => {
		// Commit originals synchronously with a "processing"
		// status so the composer reflects the paste/drop
		// immediately and the send gate blocks dispatch. The
		// async worker below swaps in resized Files when ready.
		//
		...
		const needsResize = files.map((file) => imageNeedsResize(file, budget));
		...
		// Read text content for preview; skip oversized files.
		for (const file of files) {
		...
		void processResizes(files, needsResize, budget, providerSnapshot);
	};
